...
class Ploter:
    ...
    def workers(self, data):
        ...
        return (int(x), f'{SYSNAME} - {x} workers')
    ...
    def _plot(self, xlabel, ylabel, y_axis, z_axis, filename):
        ...
        col = {'Narwhal-HS':'tab:blue', 'Tusk':'tab:green', 'Batched-HS':'tab:orange', 'Baseline-HS':'tab:red'}[SYSNAME]
        tick = {'Narwhal-HS':'o', 'Tusk':'v', 'Batched-HS':'s', 'Baseline-HS':'d'}[SYSNAME]
        ...
        for result in self.results:
            ...
            w, label = z_axis(result)

            style = style = {7:'dashed',4:'dotted',10:'solid'} [w]

            plt.errorbar(
                x_values, y_values, yerr=y_err,  # uplims=True, lolims=True,
                marker=tick, label=label, linestyle=style, color=col
            )
            # plt.yscale('log')

        plt.legend(loc='lower center', bbox_to_anchor=(0.5, 1), ncol=2, fontsize='small')
        # plt.xlim(left=0)
    ...
    def plot_client_latency(self, z_axis):
        ...
        self._plot(
            xlabel, ylabel, self._latency, z_axis, 'latency_worker'
        )
...
if __name__ == '__main__':
    ...
    for filename in glob(join('data/HS-on-Dag/aggregated/scaling/', 'agg-4-*-0-512-1000-any-any.txt')):
    ...
    print()
    results = []
    for filename in glob(join('data/Tusk/aggregated/scaling/', 'agg-4-*-0-512-1000-any-any.txt')):
        with open(filename, 'r') as f:
            results += [f.read()]
        print(filename)
