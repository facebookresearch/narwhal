...
class Ploter:
    ...
    def max_latency(self, data):
        ...
        f = search(r'Faults: (\d+)', data).group(1)
        faults = f'({f} faulty)' if f != '0' else ''
        return (int(x), f'{SYSNAME} - Max: {float(x) / 1000:,.1f} s {faults}')
    ...
    def _plot(self, xlabel, ylabel, y_axis, z_axis, filename):
        ...
        for result in self.results:
            ...
            style = style = {4000:'dashed',10000:'dotted',10:'solid'} [w]
...
if __name__ == '__main__':
    ...
    #name = 'agg-4-x-0-512-1000-any-*.txt'
    name = 'agg-4-*-0-512-1000-any-any.txt'
    for filename in glob(join('data/HS-on-Dag/aggregated/scaling/', 'agg-4-x-0-512-1000-any-*.txt')):
        with open(filename, 'r') as f:
            results += [f.read()]
        print(filename)
    ...
    print()
    results = []
    for filename in glob(join('data/Tusk/aggregated/scaling/', 'agg-4-x-0-512-1000-any-*.txt')):
        with open(filename, 'r') as f:
            results += [f.read()]
        print(filename)
