...
class Ploter:
    ...
    def plot_latency(self, name, new=True):
        ...
        for result in self.results:
            ...
            faults, label = self._nodes(result, name)
            style = {1:'dashed',3:'dotted'} [faults]
        ...
        plt.legend(loc='upper center', ncol=2, fontsize='small', bbox_to_anchor=(0.55, 1.25))
        # plt.xlim(left=0)
        plt.ylim(bottom=0, top=45_000)
...
if __name__ == '__main__':
    ...
    for system, name in zip(['Tusk', 'HS-on-Dag', 'Batched HS', 'Baseline HS'], names):
        files = glob(f'data/{system}/aggregated/faults/agg-10-any-*.txt') + glob(f'data/{system}/aggregated/faults/agg-10-1-*.txt')

        try:
            plot = Ploter(files).plot_latency(name, new)
        except:
            print(f"WARNING: error on {files} for {system}")
            raise
        new = False

    for x in ['pdf', 'png']:
        plt.savefig(f'figures/latency_faults.{x}', bbox_inches='tight')
