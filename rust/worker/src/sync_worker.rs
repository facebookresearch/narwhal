...
use dag_core::types::{NodeID, WorkerMessage, WorkerMessageCommand};
...
pub async fn handle_sync_response(
    ...
    digest_channel: Sender<(NodeID, Digest)>,
        ...
        select! {
            ...
            msg = input_channel.recv().fuse() => {
                ...
                if let WorkerMessage::Synchronize(other_worker_id, other_key) = msg.get_message() // .get_message()
                {
                    ...
                    if let Ok(resp_handle) = net.send_message(*other_worker_id, msg_bytes).await {
