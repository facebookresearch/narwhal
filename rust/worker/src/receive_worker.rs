...
use dag_core::types::WorkerMessage;
use dag_core::types::*;
...
pub struct ReceiveWorker {
    ...
    /// Control Channel
    pub control_tx: Option<Sender<u128>>,
}
...
impl ReceiveWorker {
    ...
    pub async fn store_hash_signal(
        ...
        self.store.write(final_digest.to_vec(), val).await;
        ...
        if let Some(control_channel) = self.control_tx.as_mut() {
            let now_time = SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
                .as_millis();

            let _ = control_channel.send(now_time).await;
        }
