...
impl ManageWorker {
    pub async fn new(
        ...
        let (tx_transactions, rx_tansactions) = channel(10); // channel(inner_channel_size * 10);

        // Stage 2 : Batch into Worker Messages
        let (tx_control, rx_control) = channel(100 * inner_channel_size);
        ...
        let tx_worker_sync = tx_worker_msg_v0_sync;
        ...
        tokio::spawn(async move {
            broadcaster.start_worker_broadcast_task().await;
        });
        ...
        let mut rw_pool = ReceiveWorker::new(shard_id, rx_worker_msg_v0_peer, tx_digest, store);
