...
async fn test_sync_single_channel() {
    ...
    let (my_node_key, _) = get_keypair(&mut rng);
    ...
    let store =
        Store::new(format!(".storage_integrated_{:?}", "test_sync_single").to_string()).await;
    ...
    let (mut sync_req_tx, sync_req_rx) = channel(100);
    let (sync_resp_tx, sync_resp_rx) = channel(100);

    let sync_store =
        Store::new(format!(".storage_integrated_{:?}", "test_sync_single_sync2").to_string()).await;
    ...
    let (_shard_sycn, digest_from_sync) = rx_digest_sync.next().await.unwrap();
