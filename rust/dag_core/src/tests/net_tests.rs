...
async fn test_tx_server() -> Result<(), Box<dyn Error>> {
    ...
    let (_ip, data) = rx.next().await.unwrap();
    println!("{}", data.len());
...
fn fixture_get_4nodes() -> Vec<NodeID> {
    let mut rng = StdRng::from_seed([0; 32]);
    (0..4).map(|_| get_keypair(&mut rng).0).collect()
}
...
async fn test_broadcast_client_only() -> Result<(), Box<dyn Error>> {
    ...
    let (mut input_tx, mut local_rx, mut b) = fixture_bradcaster(8983);

    // Now start the broadcaster
    tokio::spawn(async move {
        println!("Start broadcast service...");
        b.start_watchdog_tasks().await;
    });

    tokio::task::yield_now().await;
    let mut rng = StdRng::from_seed([0; 32]);
    let (node_key1, _) = get_keypair(&mut rng);
    let (node_key2, _) = get_keypair(&mut rng);
    let (node_key3, _) = get_keypair(&mut rng);
...
async fn test_broadcast_client_one_faulty() -> Result<(), Box<dyn Error>> {
    ...
    let (mut input_tx, mut local_rx, mut b) = fixture_bradcaster(8984);
...
async fn test_broadcast_client_late_start() -> Result<(), Box<dyn Error>> {
    ...
    let (mut input_tx, mut local_rx, mut b) = fixture_bradcaster(8985);
